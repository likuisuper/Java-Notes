...
内部使用了一个FIFO的双向队列，并使用两个指针分别指向头节点和尾节点。数据结构如下图
...
1、首先调用`tryAcquire`尝试获取同步资源，该方法由具体的子类实现
...
3、回到`acquire`方法中。现在已经将一个Node放入到了等待队列尾部，而处于等待队列的节点是从头节点一个一个去获取资源的。具体的实现在`acquireQueued`方法中
...
可以发现，设置当前节点为头节点是调用了`setHead`方法，该方法不是CAS的，因为只有一个线程能够成功获取到同步状态。
