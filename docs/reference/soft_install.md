...
如果在network-scripts目录中不存在ifcfg-ens33，而是ifcfg-wired_xxx这种的话，那么是一样的，不同的地方在于**不仅要修改ip地址，还要将mac地址改了，也就是HWADDR这个配置，它默认值是被克隆的虚拟机物理地址，要将它改成当前虚拟机的物理地址。右键当前虚拟机，找到网卡，然后高级设置中复制粘贴即可，否则会报错**。
